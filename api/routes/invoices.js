...
router.get("/", async (req, res, next) => {
  try {
    ...
    if (search) {
      clauses.push(
        "(i.patient_name LIKE ? OR i.invoice_number LIKE ? OR i.patient_phone LIKE ?)"
      );
...
router.post("/", async (req, res, next) => {
  ...
  try {
    ...
    const [testRows] = await conn.query(
      `SELECT * FROM tests_catalog WHERE id IN (${test_ids
        .map(() => "?")
        .join(",")})`,
      test_ids
    );
    ...
    await conn.query("UPDATE invoices SET invoice_number = ? WHERE id = ?", [
      invoiceNumber,
      invoiceId,
    ]);
...
router.get("/:id", async (req, res, next) => {
  try {
    ...
    if (!invoice)
      return res
        .status(404)
        .render("error", { title: "404", message: "الفاتورة غير موجودة" });
